"""Optuna optimizer v0.8.5 — walk-forward validation (70% train / 30% OOS).

Changelog:
  v0.8.5 — BUG 23 FIX: sl_pct >= tp1_pct bloca combinatii valide 1:1 RR.
    Constraint prea strict: RR 1:1 (sl == tp1) e valid cu trailing dupa.
    Fix: sl_pct > tp2_pct (daca SL mai mare decat TP2 nu are sens geometric).
    Constraint tp2_pct > tp1_pct pastrat intact.

  v0.4.1 — walk-forward 70%/30%, OOS validation.
"""
...
def optimize(
    ...
    def objective(trial: "optuna.Trial") -> float:  # type: ignore
        ...
        # Constraint 1: TP2 trebuie sa fie mai mare decat TP1
        if profile["tp2_pct"] <= profile["tp1_pct"]:
            return -999.0

        # BUG 23 FIX: inlocuit sl_pct >= tp1_pct cu sl_pct > tp2_pct
        # Motivare: SL == TP1 e valid (RR 1:1 cu trailing dupa)
        # SL > TP2 nu are sens geometric (dai SL inainte sa atingi orice TP)
        if profile["sl_pct"] > profile["tp2_pct"]:
            return -999.0
