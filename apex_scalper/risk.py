"""Risk manager v0.3: spread filter, depth check, session time filter, daily loss."""
...
# Session hours (UTC): avoid trading low-liquidity hours (e.g. 22:00-02:00 UTC)
...
class RiskManager:
    def can_open(self) -> bool:
        ...
        if state.daily_pnl <= -config.daily_loss_limit_usdt:
            logger.warning("Daily loss limit — pausing")
            state.paused = True
            return False
